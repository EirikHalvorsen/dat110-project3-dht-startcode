...
public class ChordLookup {
	...
	private NodeInterface findHighestPredecessor(BigInteger ID) throws RemoteException {
		...
		List<NodeInterface> fingerTables = node.getFingerTable();
		...
		for (int i = fingerTables.size() - 1; i >= 0; i--) {
			...
			NodeInterface finger = Util.getProcessStub(fingerTables.get(i).getNodeName(),
					fingerTables.get(i).getPort());

			// check that finger is a member of the set {nodeID+1,...,ID-1} i.e. (nodeID+1
			// <= finger <= key-1) using the ComputeLogic
			boolean inInterval = Util.checkInterval(finger.getNodeID(), node.getNodeID().add(BigInteger.ONE),
